import { IComment, PaginatedResponse } from "../../../types/types";
import { apiSlice } from "../apiSlice";

export const commentsApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    getAllComments: builder.infiniteQuery<
      PaginatedResponse<IComment>, // type of the response
      number, // queryArg = postId
      number // pageParam = page number
    >({
      ...
      query: ({ queryArg: postId, pageParam }) => {
        return {
          url: `/comments/?postId=${postId}&page=${pageParam}`,
          method: "GET",
        };
      },

      providesTags: (result, _error, postId) => {
        const comments = result?.pages.flatMap((page) => page.results) ?? [];
        return comments
          ? [
    ...
    getCommentCount: builder.query<{ postId: number; count: number }, number>({
      query: (postId) => ({
        url: "/comments/count/",
        params: { postId },
      }),
    ...
    createComment: builder.mutation<
      void,
      { commentDescription: string; post: number }
    >({
      query: (body) => ({
        url: "/comments/",
        method: "POST",
      ...
      invalidatesTags: (_res, _err, body) => [
        { type: "comments", id: body.post },
        { type: "commentCount", id: body.post },
      ],
    }),
